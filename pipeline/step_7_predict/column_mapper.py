...
def load_column_mapping(run_id: str) -> Optional[Dict[str, Any]]:
    """
    Load the column mapping information saved during model training.
    ...
    try:
        ...
        if column_mapping:
            log.info(f"Loaded column mapping with {len(column_mapping.get('encoded_columns', []))} encoded columns")
...
def encode_user_input(user_input: Dict[str, Any],
    ...
    """
    Main function to encode user input from original format to model-expected format.

    This function:
    1. Loads the column mapping saved during training
    ...
    try:
        ...
        column_mapping = load_column_mapping(run_id)
        if not column_mapping:
            issues.append("Could not load column mapping from training phase")
            return None, issues
        ...
        log.info(f"Successfully encoded user input to DataFrame with shape {encoded_df.shape}")
        log.info(f"Final columns: {list(encoded_df.columns)}")

        return encoded_df, issues

    except Exception as e:
        error_msg = f"Failed to encode user input: {str(e)}"
        log.error(error_msg)
